...
window.__SAKURA_PERF_PROFILE__ = (() => {
  ...
  return Object.freeze({
    ...
    ecoDpr: veryConstrained ? .24 : constrained ? .30 : .38,
...
(() => {
  ...
  function formatTime(date) {
    return new Intl.DateTimeFormat(undefined, {timeZone:state.timeZone,hour:'numeric', minute:'2-digit'}).format(date);
  }
  ...
  function applyTheme(theme,{save=true}={}){
    ...
    const preview=THEME_PREVIEWS[next]||THEME_PREVIEWS.sakura;const metaTheme=document.querySelector('meta[name="theme-color"]');if(metaTheme&&preview?.bg)metaTheme.setAttribute('content',preview.bg);
  ...
  function inferLocation(){
    ...
    state.timeZone=zone;
    state.coords={lat:c.lat,lng:c.lng,precise:false};
    state.locationSource='timezone';
  ...
  function requestPreciseLocation(showFeedback=true){
    ...
    navigator.geolocation.getCurrentPosition(position=>{
      state.coords={lat:position.coords.latitude,lng:position.coords.longitude};state.timeZone=Intl.DateTimeFormat().resolvedOptions().timeZone||state.timeZone||'UTC';state.placeLabel='Precise local sky';state.locationSource='precise';
      sessionStorage.setItem('sd-sky-coords-v3',JSON.stringify(state.coords));updateSky(true);showToast('Sun and moon are precisely synced. Coordinates stay in this browser session and are never displayed.');
  ...
  const EVIDENCE = {
    ...
    systems:'Additional applied tooling includes SQL/PostgreSQL, Git/GitHub, Linux, JavaScript/TypeScript, HTML/CSS, AWS EC2/EBS/security groups/remote access, and ongoing ROS 2, SLAM, NVIDIA Isaac Sim/Isaac Lab, C++, and simulation learning. Present ongoing learning as active development rather than production expertise.',
...
I understand this time is not confirmed until you reply. I am also creating a calendar invitation so you can RSVP directly.`;
  ...
  renderThemeGallery($('#quick-theme-grid'),activeThemeFilter);renderThemeGallery($('#settings-theme-grid'));
  const themeSelect=$('#setting-theme');if(themeSelect){themeSelect.innerHTML=THEME_META.map(([id,name])=>`<option value="${id}">${name}</option>`).join('')}
  ...
  const translationOriginals=new WeakMap();let pageTranslator=null,pageTranslatorTarget='';
  function translationNodes(scope){
    if(!scope)return[];const nodes=[],walker=document.createTreeWalker(scope,NodeFilter.SHOW_TEXT,{acceptNode(node){const text=node.nodeValue?.trim();if(!text||text.length<2)return NodeFilter.FILTER_REJECT;const parent=node.parentElement;if(!parent||parent.closest('script,style,code,pre,kbd,textarea,input,select,option,canvas,svg,[translate="no"],.theme-popover,.modal'))return NodeFilter.FILTER_REJECT;if(/^[-–—•·→↗＋×✓⌂▣⚙▤✉☆⌘✪✦♟◫文⌁↻♫◎]+$/.test(text))return NodeFilter.FILTER_REJECT;return NodeFilter.FILTER_ACCEPT}});let node;while((node=walker.nextNode()))nodes.push(node);return nodes}
  function restoreEnglish(){translationOriginals.forEach?.(()=>{});document.querySelectorAll('[data-sakura-translated]').forEach(el=>el.removeAttribute('data-sakura-translated'));for(const scope of [$('.sidebar'),$('.topbar'),$('.view.is-active'),$('.mobile-dock')]){translationNodes(scope).forEach(node=>{const original=translationOriginals.get(node);if(original!=null)node.nodeValue=original})}state.language='en';try{localStorage.setItem('sd-language','en')}catch{}const status=$('#language-status');if(status)status.textContent='English restored.'}
  ...
  $$('[data-density-choice]').forEach(button=>button.addEventListener('click',()=>setDensity(button.dataset.densityChoice)));$$('[data-glow-choice]').forEach(button=>button.addEventListener('click',()=>setGlow(button.dataset.glowChoice)));
  ...
  setFocusMode(state.focusMode,{silent:true});setDensity(state.density,{silent:true});setGlow(state.glow,{silent:true});renderLanguageGrid();
...
(() => {
  ...
  let quality='eco';try{quality=localStorage.getItem('sd-v24-quality')||'eco';if(!localStorage.getItem('sd-v24-quality'))localStorage.setItem('sd-v24-quality','eco')}catch{}
  ...
  function chooseDpr(){
    const native=devicePixelRatio||1,perf=window.__SAKURA_PERF_PROFILE__||{worldDpr:.88};
    // Preserve the approved artwork while spending AUTO mode on smooth motion.
    if(quality==='high')return Math.min(native,.96);
    if(quality==='eco')return Math.min(native,perf.ecoDpr||.34);
    const pixels=innerWidth*innerHeight;
    const sizeCap=pixels>3000000?.48:pixels>2000000?.54:pixels>1200000?.60:.66;
    const cap=Math.min(perf.worldDpr,sizeCap,autoDprCap);
  ...
  function resize(){
    ...
    const perf=window.__SAKURA_PERF_PROFILE__||{worldFps:24,ecoFps:18};frameBudget=quality==='high'?1/30:quality==='eco'?1/(perf.ecoFps||18):1/perf.worldFps;baseDirty=true;
  }
  ...
  let activeWorldTimeZone=Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC',lastZoneCheck=0;
  ...
  function render(perf){
    ...
    if(quality==='auto'){const desired=document.body.dataset.route==='home'?1/(worldPerf.worldFps||30):1/12;if(Math.abs(frameBudget-desired)>.0005)frameBudget=desired}
    ...
    if(perf-lastZoneCheck>30000){lastZoneCheck=perf;const z=Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC';if(z!==activeWorldTimeZone&&!sessionStorage.getItem('sd-sky-coords-v3')){activeWorldTimeZone=z;coords=inferCoords();baseDirty=true}}
