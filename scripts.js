...
(() => {
  ...
  function applyTheme(theme,{save=true}={}){
    ...
    if(save)try{localStorage.setItem('sd-theme',next)}catch{}
...
I understand this time is not confirmed until you reply. I am also creating a calendar invitation so you can RSVP directly.`;
  ...
  $$('.theme-filter-row [data-theme-filter]').forEach(button=>button.addEventListener('click',()=>{
    activeThemeFilter=button.dataset.themeFilter||'All';
  ...
  if(themeSelect){
    const groups=['Signature','Professional','City','Editor'];
  ...
  async function translateVisibleRoute(target,{silent=false}={}){
    ...
    if(!('Translator' in self)){if(!silent)openExternalTranslation(target);return false}
    const status=$('#language-status');if(status&&!silent)status.textContent='Preparing on-device translation…';
    try{
      const options={sourceLanguage:'en',targetLanguage:target};const availability=await Translator.availability(options);if(availability==='unavailable'){if(!silent)openExternalTranslation(target);return false}
      ...
      const unique=[...new Set(nodes)].slice(0,180);let done=0;
      for(let i=0;i<unique.length;i+=4){await Promise.all(unique.slice(i,i+4).map(async node=>{const original=translationOriginals.get(node)??node.nodeValue;translationOriginals.set(node,original);try{node.nodeValue=await pageTranslator.translate(original)}catch{node.nodeValue=original}done++}));if(status&&!silent)status.textContent=`Translating… ${Math.round(done/unique.length*100)}%`;await new Promise(r=>setTimeout(r,0))}
      state.language=target;try{localStorage.setItem('sd-language',target)}catch{}if(status)status.textContent='Translated locally in this browser. Proper names and technical identifiers stay unchanged.';return true
    }catch(error){console.warn('On-device translation unavailable',error);if(!silent)openExternalTranslation(target);return false}
  }
  function openExternalTranslation(target){
    const status=$('#language-status');if(location.protocol==='file:'||/^(localhost|127\.0\.0\.1)$/.test(location.hostname)){if(status)status.textContent='Full-page fallback needs the published HTTPS site. On supported Chrome desktop, on-device translation can still work locally.';showToast('Full-page translation fallback needs the published site.');return}
    const translated=`https://translate.google.com/translate?sl=en&tl=${encodeURIComponent(target)}&u=${encodeURIComponent(location.href)}`;window.open(translated,'_blank','noopener,noreferrer');if(status)status.textContent='Opened a full-page translated view in a new tab.'
  }
  ...
  setFocusMode(state.focusMode,{silent:true});setDensity(state.density,{silent:true});setGlow(state.glow,{silent:true});setSurface(state.surface,{silent:true});setFocusRing(state.focusRing,{silent:true});setTextComfort(state.textComfort,{silent:true});setSignalLens(state.signalLens,{silent:true});renderLanguageGrid();

  applyTheme(state.theme,{save:false});$('#setting-theme').value=state.theme;$('#setting-sound').checked=state.sound;$('#setting-motion').checked=state.reducedMotion;$('#setting-intro').checked=state.alwaysIntro;setSound(state.sound);
  ...
  function init(){
    ...
    populateTimeZones();
    setDensity(state.density,{silent:true});setGlow(state.glow,{silent:true});setSurface(state.surface,{silent:true});setFocusRing(state.focusRing,{silent:true});setLinkClarity(state.linkClarity,{silent:true});setTextComfort(state.textComfort,{silent:true});setSignalLens(state.signalLens,{silent:true});setSceneResolution(state.sceneResolution,{silent:true});setFpsPreference(state.fpsPreference,{silent:true});setTimeFormat(state.timeFormat,{silent:true});setTemperatureUnit(state.temperatureUnit,{silent:true});setTemperatureEnabled(state.temperatureEnabled,{silent:true});
    ...
    runIntro(false);
    // Exact sunrise/sunset needs coordinates. Reuse permission silently when
    // already granted; otherwise ask once per browser session after the intro.
    if(!sessionStorage.getItem('sd-sky-coords-v3')){
      const askOnce=()=>{if(sessionStorage.getItem('sd-sky-permission-asked-v1'))return;sessionStorage.setItem('sd-sky-permission-asked-v1','1');setTimeout(()=>requestPreciseLocation(false),5200)};
      if(navigator.permissions?.query){
        navigator.permissions.query({name:'geolocation'}).then(status=>{if(status.state==='granted'){requestPreciseLocation(false);return}if(status.state==='prompt')askOnce()}).catch(askOnce);
      }else askOnce();
    }
  }
  init();
